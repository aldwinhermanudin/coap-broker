...
int optionRegister(coap_resource_t **resource , char* temp_str){
	...
	if (upper_status && type_size > 0 && data_size > 0){
		char* opt_type = (char*) malloc(sizeof(char) * (type_size+1));
		char* opt_data = (char*) malloc(sizeof(char) * (data_size+1));
		int status = parseOption(temp_str, opt_type,opt_data);
		if(status){
			coap_add_attr(*resource, (unsigned char*)opt_type, strlen( opt_type), (unsigned char*)opt_data, strlen(opt_data), 0);
		}
		else {
			free(opt_type);
			free(opt_data);
...
int pathRegister(coap_resource_t *old_resource, coap_resource_t **new_resource , char* temp_str){
	...
	if (calPathSize(temp_str) > 0){
		...
		char* rel_path = (char*) malloc(sizeof(char) * (calPathSize(temp_str)+1)); 
		int status = parsePath(temp_str, rel_path);
		if(status){
			char* abs_path =(char*)  malloc(sizeof(char) * (total_size + 1));
			char* parent_path =(char*)  malloc(sizeof(char) * (old_resource->uri.length + 2));
			snprintf(parent_path,old_resource->uri.length+1, "%s", old_resource->uri.s);
			sprintf(abs_path,"%s/%s", parent_path, rel_path);
			free(parent_path);
			*new_resource = coap_resource_init((unsigned char*) abs_path, strlen(abs_path), COAP_RESOURCE_FLAGS_RELEASE_URI); 
		}
		free(rel_path);
		return status;
	}
...
int parseOptionURIQuery(char* option_value, unsigned short option_length, char** query_name, char** query_value){
	char* temp_uri_query = (char*)  malloc(sizeof(char) * (option_length+2));
	...
	if (upper_status && name_size > 0 && value_size > 0){
		*query_name = (char*) malloc(sizeof(char) * (name_size+1));
		*query_value = (char*) malloc(sizeof(char) * (value_size+1));
